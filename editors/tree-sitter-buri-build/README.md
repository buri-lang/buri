...
EBNF exists to generate it from: the dialect is whatever `textproto.rs` reads,
and that reader is small enough to mirror rule for rule.
...
them — see [Publishing](#publishing).
...
`ERROR` and zero `MISSING` nodes. This corpus needs no recorded verdicts the way
the Buri grammar's does: a build file here is one the toolchain already reads,
or the tests around it would not run.

The `REFUSED` list in `check.sh` covers the other direction. It holds inputs
`textproto.rs` turns away, and the syntax tree has to turn them away too, which
is what says the grammar has not quietly become more permissive than the reader.
`// not a comment` is on the list, because that is the mistake this whole
