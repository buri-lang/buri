...
//!
//! `conformance.rs` drives the same corpus through `buri test`, which is the
...
mod native {
    ...
    const PACKAGES: &[Case] = &[
        ...
        excluded(
            ...
            "core/map is `list.fold`, `list.filterCtx` and `list.sortBy` over \
             ...
             (`native_cranelift.rs`) — so what defers maps is the closure walk \
    ...
    /// the same reason `conformance.rs` counts its assertions.
    #[test]
    fn the_native_set_passes() {
        ...
        for case in PACKAGES.iter().filter(|c| c.excluded.is_none()) {
            ...
            // corpus is shared with `conformance.rs` and may be mid-change.
            match missing_for(&source) {
                Err(e) => {
    ...
    ///
    /// `conformance.rs` has the same test for the same reason: a suite that
