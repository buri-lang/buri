...
//!
//! `doctest.rs` compiles what the documents *show*. This file checks what the
...
use buri::{doc_assemble, doc_md, doc_topics};
...
fn every_document_is_scannable() {
    ...
    for doc in DOCUMENTS {
        let text = read(doc);
        if let Some(line) = doc_md::unterminated_fence(&text) {
            problems.push(format!("{doc}:{line}: this ``` is never closed"));
        }
        for f in doc_md::fences(&text) {
            if let Err(msg) = doc_md::parse_info(f.raw_info) {
...
fn the_fence_census_is_what_we_think_it_is() {
    ...
    for doc in DOCUMENTS {
        for f in doc_md::fences(&read(doc)) {
...
fn every_link_resolves() {
    ...
    for doc in DOCUMENTS {
        ...
        let anchors: Vec<String> =
            doc_md::headings(&text).iter().map(|h| doc_md::slug(h.title)).collect();
        ...
        for link in doc_md::links(&text) {
            ...
            let other_anchors: Vec<String> =
                doc_md::headings(&other).iter().map(|h| doc_md::slug(h.title)).collect();
...
fn the_assembled_documents_are_not_stale() {
    let drifted = doc_assemble::drifted(&repo_root());
...
fn every_grammar_keyword_is_a_keyword() {
    let grammar = doc_topics::GRAMMAR;
    let mut missing = Vec::new();
    for kw in buri::lex::Kw::ALL {
...
fn every_topic_is_servable() {
    for source in buri::doc::sources() {
...
fn the_standard_library_reference_is_complete() {
    ...
    let modules = buri::doc_api::from_loaded(&analysis.loaded, &buri::doc_api::std_filter);

    assert_eq!(modules.len(), buri::stdlib::MODULES.len(), "a module is missing from the reference");
...
fn documented_items_carry_their_documentation() {
    ...
    for m in &modules {
        ...
        let page = buri::doc_api::render(m);
        for item in &m.items {
...
fn a_module_doc_comment_must_come_first() {
    ...
    let parsed = buri::parse::parse(map.text(file), file);
    assert!(
...
fn every_error_page_is_provoked_by_its_own_example() {
    ...
    for e in buri::doc_errors::ERRORS {
        let doc = format!("cli/src/docs/errors/{}.md", e.code);
        failures.extend(buri::doctest::run_file_at(&repo_root(), &doc, e.text));
    }
    assert!(
        failures.is_empty(),
        "{} error page(s) do not provoke what they describe:\n\n{}",
        failures.len(),
        buri::doctest::report(&failures)
    );
}
...
fn the_catalog_and_the_compiler_agree() {
    ...
    let missing: Vec<&String> =
        emitted.iter().filter(|c| buri::doc_errors::find(c).is_none()).collect();
