...
| `recovery` | That one mistake reads as one mistake. Every compiling source in the repository, with a token deleted, inserted or exchanged, held to six invariants: one diagnostic per mistake, its caret at the mistake, its `fix` naming the token, no type error invented downstream, and the file still formatting. Plus a curated case per construct-and-mutation pair pinning the exact message, span and edit — every construct the grammar has against a missing closer, a stray token, a swapped pair, a deleted `,`, an unterminated string, a missing `;`, a keyword where a name goes, and a truncated file. A case whose recording reads wrong carries `// ISSUE:` naming what was filed about it. Each invariant is held per mutation shape. Where the mutated text has a second reading the grammar accepts, it is held against a ceiling rather than zero, and `ceiling()` states which rows those are. A ceiling is a **percentage of the row's population**, read off a `BURI_RECOVERY_CAP=0` run, so a source landing in the repository cannot tip a row whose per-case behaviour did not change. |
