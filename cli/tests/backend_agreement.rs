...
//!
//! `cargo test -p buri --features backend-llvm --test backend_agreement` is the
...
//!
//! With `--no-default-features` there is no native backend and the file
//! compiles to nothing. With one but no runtime archive, no `cc`, or no
//! JavaScript engine, every test returns early with a printed reason:
//! `native_ready` is the same gate `buri build` uses.
//!
//! # What is not here, and why
//!
//! * **The exhaustive float corpus.** `native_float_parity.rs` sweeps 3.8
...
//!   That is `native_conformance.rs`'s pattern, for its reason.
...
"#,
    ...
    ///
    /// `native_float_parity.rs` is the corpus — 3.8 million doubles through
...
"#;
    ...
    /// `native_conformance.rs` names against every package that has it — so
...
"#,
    ...
    /// true. The same relationship `native_conformance.rs`'s
    ...
    fn every_row_of_the_table_names_a_test_that_exists() {
        ...
        let me = include_str!("backend_agreement.rs");
            ...
            for name in names {
                assert!(
                    ...
                    "§12 row {} is pinned by `{name}`, and there is no such test in \
                     `cli/tests/backend_agreement.rs`",
                    number.trim()
                );
            }
        }
