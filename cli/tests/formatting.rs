...
/// The same, for the generated corpus, which is a floor on the sampler rather
/// than on the path: a rule that stopped choosing cases would otherwise leave
/// a suite that passes by having nothing to run.
const GENERATED_FLOOR: usize = 400;

/// How many generated pairs are checked in.
///
/// **The one constant that scales the corpus.** The sampler fills every
/// coverage cell it can and then thins them round-robin to this many, so
/// raising it adds cases to the shapes that have more to say and lowering it
/// takes them away evenly. `BURI_BLESS=1 cargo test -p buri --test formatting`
/// rewrites the tree to match.
const GENERATED_TOTAL: usize = 1_000;

fn cases() -> Vec<PathBuf> {
    let mut all = case_dirs(&tests_dir().join("formatting"), "input.buri", FLOOR);
    all.extend(generated_cases());
    all
}

/// The generated pairs, written first if this run is a blessing one.
///
/// `cases()` is what every test in this file starts from, so generation hangs
/// off it rather than off a test of its own: `cargo test` runs test functions
/// in whatever order it likes, and a corpus that had to be regenerated by one
/// test before another read it would be a race.
fn generated_cases() -> Vec<PathBuf> {
    ...
    case_dirs(&generated_dir(), "input.buri", GENERATED_FLOOR)
}

fn generated_dir() -> PathBuf {
    tests_dir().join("formatting/generated")
}
...
/// Where the sample starts. The same seed `recovery.rs` draws its invariant
/// population with, so a case pinned here is a case that population holds.
const BASE_SEED: u64 = 0x0B00_1A57_5EC0_4E27;

/// Every claim this suite makes about a `recovery_` case, asked of one
/// mutation before it is checked in.
///
/// A candidate that fails one of them is passed over for the next in its cell,
/// and the counts are printed: a corpus that had to skip a hundred candidates
/// to find its cases is saying something about the formatter, and the number
/// is where it says it. The margin is the only expected skip — a mutation can
/// put a string literal where the formatter has to lay it out, which is the
/// question `width_*` asks and not this one.
fn recordable(text: &str) -> Result<String, &'static str> {
    let before = buri::parsing::parser::parse(text, FileId(0)).errors.len();
    if before == 0 {
        return Err("the mutation left a program that parses");
    }
    let Some(out) = source(text) else { return Err("the formatter refused the file") };
    if source(&out).as_deref() != Some(out.as_str()) {
        return Err("formatting the output again moved it");
    }
    if buri::parsing::parser::parse(&out, FileId(0)).errors.len() != before {
        return Err("the output has a different number of syntax errors");
    }
    if region_text(text) != region_text(&out) {
        return Err("a region came back changed");
    }
    if comments(text) != comments(&out) || tokens(text) != tokens(&out) {
        return Err("a comment or a token was invented or lost");
    }
    let regions = buri::formatting::broken_regions(&out);
    let mut at = 0usize;
    for line in out.lines() {
        ...
        if line.chars().count() > 88 {
            return Err("a line the formatter laid out is over the margin");
        }
    }
    Ok(out)
}

/// The mutations this corpus is made of, and why the others were passed over.
fn picks() -> (Vec<pinned::Pick>, std::collections::BTreeMap<&'static str, usize>) {
    let sources = mutation::corpus(&harness::repo_root());
    assert!(sources.len() > 100, "the seed corpus is not reachable: {}", sources.len());
    let mut skipped: std::collections::BTreeMap<&'static str, usize> =
        std::collections::BTreeMap::new();
    let mut keep = |m: &mutation::Mutation| match recordable(&m.source) {
        Ok(_) => true,
        Err(why) => {
    ...
    let picks = pinned::select(&sources, BASE_SEED, GENERATED_TOTAL, &mut keep);
    (picks, skipped)
}

/// Writes `generated/<case>/input.buri` for every case the sampler chose, and
/// deletes the directories it no longer chooses.
fn write_generated_inputs() {
    ...
    if let Ok(entries) = std::fs::read_dir(&dir) {
        for e in entries.filter_map(Result::ok) {
            ...
            if e.path().is_dir() && !wanted.contains(&n) {
...
///
/// The pairs are blessed, and a blessed file is only as good as the question
/// it answers. This is that question written down: the sampler, run again from
/// the same seed over the same sources, chooses exactly these cases and writes
/// exactly these bytes. A case edited by hand, a case left behind by a
/// narrowed rule, and a corpus whose seed drifted all fail here.
///
/// The *outputs* are regenerated by the golden tests above, which recompute
/// every `expected.buri` and compare it — so between this test and those,
/// `BURI_BLESS=1 cargo test -p buri --test formatting` is a byte-for-byte
/// idempotent operation.
#[test]
fn the_generated_corpus_is_what_the_generator_writes() {
    // Through `cases()`, so that a blessing run has written the tree before
    // this reads it — the same reason generation hangs off `cases()` at all.
    let _ = cases();
    let (picks, skipped) = picks();
    let dir = generated_dir();
    ...
    assert!(
        ...
        "the generated corpus is not what the sampler chooses. \
         `BURI_BLESS=1 cargo test -p buri --test formatting` rewrites it.\n  \
         checked in and no longer chosen ({}): {:?}\n  chosen and not checked in ({}): {:?}",
        stale.len(),
    ...
    for p in &picks {
        let path = dir.join(&p.name).join("input.buri");
        if read(&path) != p.mutation.source {
            differ.push(p.name.clone());
        }
    }
    assert!(
        differ.is_empty(),
        "{} generated input(s) are not what the mutator writes: {:?}",
        differ.len(),
        differ.iter().take(8).collect::<Vec<_>>()
    );
    assert!(
        picks.len() >= GENERATED_FLOOR,
        "the sampler chose {} cases, and the floor is {GENERATED_FLOOR}",
        picks.len()
    );
    let rows = pinned::coverage(&picks);
    eprintln!(
        "formatting: {} generated pairs over {} shapes, passed over {skipped:?}\n{}",
        picks.len(),
        rows.len(),
