...
// ---------------------------------------------------------------------------
// The heap check, on every process this harness starts
// ---------------------------------------------------------------------------
//
// **Every Buri program a suite in this repository runs is asked, on its way
// out, whether it gave back every block it took.** `cli/tests/native/main.rs`
// is the long argument for why: `middle::rc` is a whole-program analysis whose
// failures are silent, five of them have shipped, and every one was found by a
// person noticing a wrong answer. The native domain has asked this of every
// program it runs for a while; what was missing is the other half of the
// population — the programs a *user's* commands produce, which is what the
// suites that drive the real `buri` binary run.
//
// So it is one variable, set here rather than per suite, and one assertion,
// made where a `Run` is built rather than in whatever the caller went on to
// assert. The two together are what make it impossible for a new test to run a
// Buri program without the question being asked:
//
//   * `buri run` hands a native artifact this process's own environment, so the
//     artifact is checked with nothing further to arrange;
//   * `buri test` forwards the variable into the test binaries it spawns
//     (`cli/src/commands/test.rs`'s `HEAP_CHECK`) and reports a failure as a
//     diagnostic naming the suite, which lands in the same `Run`;
//   * a suite that runs a built artifact itself uses [`heap_checked`].
//
// A JavaScript artifact is unaffected: the check is the native runtime's, and
// `bun` ignores the variable. That is the one hole, and it is the backend's
// rather than this harness's.

/// The mode every process here runs under: the exit audit **and** the
/// quarantine.
///
/// `1` rather than `leak`, which would be the audit alone. Two reasons, and the
/// first is that it costs nothing measurable — the programs these suites run
/// are small, and a `buri test //...` over the worked repository takes the same
/// 0.48s under either. The second is that `cli/tests/native/shared.rs` already
/// runs its whole corpus under `1`, and one answer to "which mode does this
/// repository check in" is worth more than a saving nobody can measure.
pub const HEAP_CHECK: (&str, &str) = ("BURI_RT_HEAP_CHECK", "1");

/// The status `cli/runtime/memory.rs` stops a program with when the check
/// fails. Not 1, which is an ordinary abort's, and not a signal.
pub const HEAP_CHECK_STATUS: i32 = 97;

/// The prefix on every line the check prints.
pub const HEAP_CHECK_LINE: &str = "buri heap check:";

/// A `Command` for the `buri` binary, under the heap check.
///
...
/// The same, for a program the suite built and is running itself.
pub fn heap_checked(cmd: &mut Command) -> &mut Command {
    cmd.env(HEAP_CHECK.0, HEAP_CHECK.1)
}

/// What the heap check said about a run, if what it said was a failure.
///
/// Two signs, because a run reaches a caller two ways. A program run directly
/// — by `buri run`, or by a suite — *is* the process, so its status is the
/// check's. A program `buri test` ran is a child, and what comes back is the
/// runner's own exit code with the check's sentence in the diagnostic; so the
/// text is read too, and read for a line that is not the `ok` receipt
/// `BURI_RT_HEAP_REPORT` prints on a clean audit.
pub fn heap_check_failure(code: i32, text: &str) -> Option<String> {
    let said = text
        .lines()
        .find(|l| l.contains(HEAP_CHECK_LINE) && !l.contains("buri heap check: ok"))
        .map(str::trim);
    match (code == HEAP_CHECK_STATUS, said) {
        (_, Some(line)) => Some(line.to_string()),
        (true, None) => Some(format!(
            "a process exited {HEAP_CHECK_STATUS} — the heap-check status — and said nothing"
        )),
        (false, None) => None,
    }
}
...
impl Run {
    ...
    /// **Panics if any Buri program this run started leaked a block**, or
    /// reached one it had already given back.
    ///
    /// Called where a `Run` is *built* rather than from `ok` or `exits`, and
    /// that is the whole point: a suite that expects a non-zero status — a
    /// failing test suite, a program that aborts, a refusal — asserts the
    /// status it expected and would never think to ask this. The question is
    ...
    pub fn heap_ok(&self) -> &Run {
        if let Some(said) = heap_check_failure(self.code, &self.all()) {
            panic!(
                "`{}` did not give back every block it allocated:\n    {said}\n\n\
                 A leak or a use-after-free is a bug in the toolchain rather than in this \
                 test — the code that releases a program's memory is code the compiler \
                 inserted. `BURI_RT_HEAP_CHECK=trace` prints every surviving block. What \
                 the run printed:\n{}",
                self.what,
                indent(&self.all())
            );
...
/// Every path that runs the CLI, with the heap assertion made on the way out.
///
/// The assertion is here rather than in the callers because here is the one
