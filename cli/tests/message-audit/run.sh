...
# then a deterministic spread of the generated ones. That test is an ordinary
# one — it used to be `#[ignore]`d and this script used to pass `--ignored`,
# which meant the format below was generated by something nothing ran and read
# by something CI never ran either. It runs in the suite now and asserts the
# five fields the `awk` further down pulls out, so the two halves of this
# format fail together instead of drifting apart.
#
...
(
  ...
  BURI_MESSAGE_AUDIT_SAMPLED="$sampled" \
    cargo test -p buri --test recovery -- --nocapture --exact \
      message_audit_corpus \
    >/dev/null
)
