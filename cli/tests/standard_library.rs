...
///
/// The two ways to satisfy it: put the module in `standard_library::EAGER_MODULES`, or
...
fn a_lazily_loaded_module_declares_no_method_on_a_built_in_type() {
    ...
    for path in buri::compiler::standard_library::MODULES {
        if buri::compiler::standard_library::EAGER_MODULES.contains(path) {
            continue;
        }
        let Some(source) = buri::compiler::standard_library::source(path) else { continue };
        checked += 1;
    ...
    assert!(
        ...
        "a lazily loaded module declares a method on a built-in type, which will not \
         ...
         standard_library::EAGER_MODULES, or make these free functions.",
        offenders.join("\n  ")
    );
