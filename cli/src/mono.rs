...
use crate::hir::{self, ExprKind, PatKind};
use crate::types::*;
use std::collections::HashMap;

/// One concrete function in the output. `hir::ExprKind::CallFn`'s `func` field
...
impl<'a> Monomorphizer<'a> {
    ...
    fn rewrite(&mut self, mut e: hir::Expr, targs: &[Ty]) -> hir::Expr {
            ...
            ExprKind::Block { stmts, tail } => ExprKind::Block {
                stmts: stmts
                    ...
                    .map(|s| match s {
                        ...
                        hir::Stmt::Expr(x) => hir::Stmt::Expr(self.rewrite(x, targs)),
                    })
                    .collect(),
    ...
    fn rewrite_all(&mut self, xs: Vec<hir::Expr>, targs: &[Ty]) -> Vec<hir::Expr> {
        xs.into_iter().map(|x| self.rewrite(x, targs)).collect()
    }

    fn rewrite_pattern(&mut self, mut p: hir::Pattern, targs: &[Ty]) -> hir::Pattern {
        ...
        if let Ty::Ctx(id) = &recv {
            ...
            if let Some(first) = args.first_mut() {
                let base = std::mem::replace(
                    first,
                    hir::Expr::new(ExprKind::Error, Ty::Error, span),
                );
                *first = hir::Expr::new(
    ...
    fn structural_call(
        ...
        let desc_arg =
            hir::Expr::new(ExprKind::Int(desc as u128, false), Ty::Error, span);
        let mut all = args;
        all.push(desc_arg);
        match (name.as_str(), method) {
            ...
            ("Show", _) => {
                ...
                // descriptor only.
                let mut trimmed: Vec<hir::Expr> = Vec::new();
    ...
    fn derived_operator(
        ...
        let unwrapped: Vec<hir::Expr> = args
            .into_iter()
            .map(|a| {
                hir::Expr::new(
            ...
            .collect();
        let computed =
            hir::Expr::new(ExprKind::Prim { op: prim_op, prim, args: unwrapped }, inner, span);
