...
pub const ERRORS: &[ErrorDoc] = &[
    ...
    e!("ctx-binding-not-allowed", "`ctx` is bound where a context may be built"),
    ...
    e!("no-ctx-in-scope", "A function that needs an effect declares `ctx`"),
    ...
    e!("package-shadows-a-module", "A module path may not also be a package path", &["build/libraries"]),
    ...
    e!("type-args-required", "A trait method call resolves to a concrete type"),
