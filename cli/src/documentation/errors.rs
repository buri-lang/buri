...
pub const ERRORS: &[ErrorDoc] = &[
    ...
    e!("generator-cycle", "A generator's tool is not built from what it generates", &["build/build-files"]),
    e!("generator-diagnostic", "A generator reports under a code the catalogue has", &["build/build-files"]),
    e!("generator-failed", "A generator answers with a response", &["build/build-files"]),
    e!("generator-without-a-tool", "A generator names the program that runs it", &["build/build-files"]),
