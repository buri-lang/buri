...
fn collect_findings(args: &cli::Args) -> Result<(Session, Diagnostics), i32> {
...
fn regenerate_build_files(s: &mut Session, diags: &Diagnostics) -> usize {
    ...
    let mut fixed = 0;
    for pkg in packages {
        match crate::gen::regenerate(s, pkg) {
            Ok(Some(update)) => {
...
/// answers was meant. The result is run through `format::source`, which returns
...
fn apply_fixes(s: &mut Session, diags: &Diagnostics) -> usize {
    ...
    let mut by_file: BTreeMap<u32, Vec<&crate::diag::Edit>> = BTreeMap::new();
    for d in &diags.items {
        for e in &d.edits {
    ...
    for (file, mut edits) in by_file {
        ...
        if !crate::parse::parse(&text, id).errors.is_empty() {
...
fn check_test_suites(s: &Session, pkg: PkgId, diags: &mut Diagnostics) {
    let p = s.ws.pkg(pkg);
    let mut report = |suite: &crate::buildfile::TestSuite, rule: &str| {
...
fn check_unreachable_exports(
    ...
    for m in &analysis.loaded.modules {
        ...
        for item in &m.ast.items {
            let (path, specs) = match item {
                crate::ast::Item::Import(i) => match &i.clause {
                    crate::ast::ImportClause::Named(sp) => (&i.path, sp),
                    crate::ast::ImportClause::Namespace(_) => {
                        taken_whole.insert(i.path.as_str());
                        continue;
                    }
                },
                crate::ast::Item::ReExport(r) => (&r.path, &r.specs),
                _ => continue,
            };
            let Ok(loc) = s.ws.resolve_module(path) else { continue };
            if loc.pkg != Some(own) {
    ...
    for m in &analysis.loaded.modules {
        ...
        if m.path.ends_with("/lib") || !matches!(m.role, crate::compile::Role::Source) {
            continue;
        }
...
fn exported_name(item: &crate::ast::Item) -> Option<(&str, Span)> {
    use crate::ast::Item;
...
fn check_unused_imports(s: &Session, m: &crate::compile::ModuleData, diags: &mut Diagnostics) {
    ...
    for item in &m.ast.items {
        match item {
            crate::ast::Item::Import(i) => import_ranges.push((i.span.start, i.span.end)),
            // A re-export names what it exports, so it *is* a use.
            crate::ast::Item::ReExport(_) => {}
            _ => {}
        }
    }

    let text = s.map.text(m.file);
    let lexed = crate::lex::lex(text, m.file);
    let mut used: BTreeSet<&str> = BTreeSet::new();
    for t in &lexed.tokens {
        let crate::lex::Tok::Ident(name) = &t.tok else { continue };
    ...
    for item in &m.ast.items {
        let crate::ast::Item::Import(i) = item else { continue };
        let specs: Vec<(&str, Span)> = match &i.clause {
            crate::ast::ImportClause::Named(specs) => {
                specs.iter().map(|sp| (sp.local().name.as_str(), sp.span)).collect()
            }
            crate::ast::ImportClause::Namespace(n) => vec![(n.name.as_str(), n.span)],
        };
        ...
        let survivors: Vec<String> = match &i.clause {
            crate::ast::ImportClause::Named(specs) => specs
                .iter()
                ...
                .collect(),
            crate::ast::ImportClause::Namespace(_) => Vec::new(),
        };
...
fn check_tests_assert(
    ...
    let mine: BTreeSet<crate::types::ModuleId> = analysis
        .loaded
        .modules
        .iter()
    ...
    for case in &analysis.checked.tests {
        ...
        while let Some(f) = queue.pop() {
            ...
            crate::hir::walk(&body.expr, &mut |e| {
                match &e.kind {
                    crate::hir::ExprKind::CallFn { func, .. } => queue.push(*func),
                    crate::hir::ExprKind::FnRef(g, _) => queue.push(*g),
                    _ => {}
                };
            });
        }
