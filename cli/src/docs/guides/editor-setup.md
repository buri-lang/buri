...
you what a build would say: diagnostics, hover, go-to-definition, references,
...
`REPO.buri` as the workspace root. The shape of it, in whatever configuration
language your client happens to use:
...
Two things a client has to get right. First, only the protocol goes to stdout,
so a wrapper that echoes anything into that stream looks like a broken editor.
The server writes its own log lines to stderr for exactly this reason. Second,
the server answers requests one at a time in arrival order, so a client that
...
`grammar.js` is generated from `cli/src/docs/grammar.ebnf`, the normative
grammar, and a cargo test compares the two byte for byte, so it cannot drift
from the language.
