...
`Float` are **aliases** for `I64` and `F64`, not a separate tier, so the two
kinds of code meet with no conversions at the boundary.
...
literals, and there are no `5u8` suffixes to learn.
...
The return type says whether a conversion can fail, so you never pick an
operator to say it. Overflow is undefined behaviour rather than silent
wrapping. Reach for `x.wrappingAdd(y)` or `x.saturatingAdd(y)` when wrapping is
the intent, and `x.checkedAdd(y)` when you want to be told.
