...
`ui/testing` is a headless platform, a renderer for looking at what a tree
became, and `snapshot`, which paints one and holds it to a golden PNG. Only a
test source may import it.
