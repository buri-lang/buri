...
**A dropped print is passed over, and it is the exception that keeps the rule usable.** `Stdout` and `Stderr` answer `Result<(), IoError>` because a closed pipe is a thing that happens, so nearly every line a program writes is a `Result` — and the failure a print reports is the one a program can least act on, because the stream it would report the failure *on* is the stream that failed. `io.println(ctx, x).ignore()` is punctuation rather than a decision, and a report dominated by it is a report nobody reads. Every other dropped `Result` is still here, `fs.writeText(...).ignore()` included: a failed write is a failure somebody chose not to look at.
