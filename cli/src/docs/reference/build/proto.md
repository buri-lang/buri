...
A `.proto` file in a package is a source, and the module it becomes is generated
rather than read. [Import a `.proto` schema](../../guides/proto.md) is the task;
this page is the mapping, and the mapping is a promise: it is what somebody
else's program will find in the bytes.
...
schema no rule lists is [`unused-library`](../lints/unused-library.md), the
...
([`core/num`](../standard-library.md)) — so a `uint64` or `int64` field
...
[the guide](../../guides/proto.md#use-the-types) has it in use.
