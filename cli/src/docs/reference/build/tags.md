...
**A tag that `REPO.buri` does not declare is an error** — `unknown-tag`, with
the nearest declared name suggested. There are no ad-hoc tags, and a `tags`
entry is never a harmless annotation.
...
A `tag-violation` names both tags, the target that carries each, the path that
reaches it, and each tag's `doc`. The path is printed because in a repository of
any size the interesting question is never "which library is tagged `server`"
but "who dragged it in." The `doc` strings are printed for the same reason: the
tag is a policy, and the policy should say why. A `platform-violation` reports
the same way. [Enforce policy with tags](../../guides/tags-policy.md) walks one
of each.

Two rules, two diagnostics, and the second one is the reason the first is not
load-bearing: a repository states its deployment policy on a tag once and gets
both — a target that drops the offending tag can still fail on the platform
whitelist the other tag carries.
...
want to be able to write. `I64` on the JS target ([a `BigInt`, not a
