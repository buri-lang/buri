---
title: A value has the type its position expects
message: expected {expected}, found {found}
---
# A value has the type its position expects
...
Those two lines are the whole story: what the position wanted, and what is
there. Only a conversion with a name earns a fix line beside them.

## Numbers do not convert on their own
...
## A `.Variant` is a variant

A leading dot takes its enum from the position it is written in, so a position
that is not an enum has nothing to give it, and `expected` is the type to write
instead. What is reported as found is a variant rather than a type, because
`.Const` on its own is not one — and guessing an enum from the payload would put
a type in front of you that nothing in your program says.
