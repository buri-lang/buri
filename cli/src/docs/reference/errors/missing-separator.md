...
editor's quick fix and `buri lint --fix` both write it for you. The parser then
reads the rest of the list rather than abandoning it.
...
One rule, stated once, also lets the parser tell a missing separator from a list
that has ended. The next element is already under the cursor, so the mistake is
one comma and the diagnostic is one sentence.
