...
it is reachable only from the module that wrote it. One rule, so one code: the
same error covers a function you cannot call, a field you cannot name, and a
variant of an enum whose own declaration is private.
...
the hidden field. That makes a private field an invariant rather than an
inconvenience, and it is how the standard library mints a type only from the
inside. `ui/effect`'s `Scope` is the worked example. The runtime hands a
reactive closure one, and no program can build one, which is what "a closure can
never capture a context" rests on.
