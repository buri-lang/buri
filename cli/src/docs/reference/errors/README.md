...
| `{because}` | The clause saying why a platform withholds a `core/host` grant. | `effect-not-on-platform` |
...
| `{effect}` | The effect's name. | `duplicate-bound`, `effect-and-trait`, `effect-not-on-platform` |
...
| `{name}` | The identifier the diagnostic is about, where no narrower role name applies. See the note below the table. | `ambiguous-free-function`, `context-not-a-value`, `dead-code`, `declaration-without-a-body`, `derive-not-a-trait`, `duplicate-field`, `duplicate-method`, `duplicate-module-declaration`, `duplicate-pattern-binding`, `effect-not-on-platform`, `effect-param-not-ctx`, `impl-fn-without-self`, `impl-outside-its-module`, `lambda-captures-effect`, `lambda-captures-generic`, `method-declared-free`, `method-not-a-value`, `missing-field-value`, `missing-payload-pattern`, `no-such-export`, `no-type-arguments`, `not-a-trait`, `not-an-effect`, `not-on-the-surface`, `oversized-function`, `proto-ambiguous-type`, `proto-duplicate-type`, `proto-unknown-type`, `too-many-parameters`, `trait-not-an-effect`, `trait-used-as-a-type`, `type-not-a-value`, `type-parameter-with-arguments`, `uninhabited`, `unresolved-name`, `unresolved-type`, `unresolved-type-in-pattern`, `unused-import`, `unused-variable`, `wrong-matched-value-count`, `wrong-value-count` |
| `{operations}` | The intrinsic operations a toolchain cannot compile, quoted and joined by `diagnostics::names`. | `cryptography-not-available`, `networking-not-available` |
| `{output}` | A declared output, spelled the way a build file and `--output` spell it — `linux/x86_64`, or `macos` where the output named no architecture. **Not a target triple**: a triple carries the host's own architecture whenever the output named none, so a recorded diagnostic holding one pins the runner rather than the product. | `native-artifact-not-available` |
...
| `{platform}` | The platform, spelled as the sentence wants it — `Platform::slug()` (`js`, `linux`) in prose, `Platform::proto()` (`JS`) where the sentence quotes a build file. | `native-run-not-available`, `output-with-an-architecture`, `platform-not-implemented`, `platform-violation` |
...
| `{platforms}` | The platforms a host effect is *not* allowed on, named inside the sentence — `Platform::sentence_phrase`, which writes the article and the plural (`the WEB platform`, `the MACOS and JS platforms`). | `effect-not-on-platform` |
...
| `{profile}` | The build profile the invocation asked for — `BuildMode::name()`, `debug` or `release`. The two have different requirements, so the sentence about a missing one has to say which was asked for. | `native-run-not-available` |
...
| `{reason}` | Why a proto construct is refused, or which of `build/actions.rs`'s three native gaps was hit (`NativeGap::reason`). | `native-artifact-not-available`, `platform-not-implemented`, `proto-unsupported` |
| `{fix}` | The whole fix sentence, supplied by the call site, where the fix depends on which of several causes the diagnostic found (`NativeGap::fix`). | `native-artifact-not-available` |
