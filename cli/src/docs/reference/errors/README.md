...
>>>>>>> worktree-agent-a444799d6aaa2136e
...
| `{platform}` | The platform, spelled as the sentence wants it — `Platform::slug()` (`js`, `linux`) in prose, `Platform::proto()` (`JS`) where the sentence quotes a build file. | `host-not-granted`, `native-run-not-available`, `output-with-an-architecture`, `platform-not-implemented`, `platform-violation` |
