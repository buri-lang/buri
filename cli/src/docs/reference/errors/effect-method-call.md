...
method has one, and the diagnostic names both it and the module it comes from.
`ctx.println("hi")` becomes `io.println(ctx, "hi")`, `ctx.readFile(p)` becomes
`fs.readText(ctx, p)`, `ctx.allocate(n)` becomes `alloc.allocate(ctx, n)`.

## Why

A context is the set of things a program may do. You write it down so a reader
can see what a function reaches for. `x.f(y)` hides that. The receiver is the
smallest, quietest part of a call, so an effect performed through one reads like
a method on an ordinary value. Passing the context as an argument puts the
...
bound searches every effect the bound declares, so two effects declaring the
same verb make that verb ambiguous for everybody who binds both. `Ui.read` and
...
Two layers sit below this line and keep the method form. The standard library
holds the wrapper functions, so its bodies are the only code that reaches an
effect at all. And the body of an `impl` that *supplies* an effect implements
the operation, which is what keeps an attenuating wrapper writable:
