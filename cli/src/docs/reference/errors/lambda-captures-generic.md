...
the type does not say so, and cannot. Inside `fn hide<T>(x: T)`, `T` is opaque,
and the compiler checks a generic body once for every instantiation at once
rather than once per call site. If the rule asked only "does this type mention
an effect?", the answer for `T` would be no. `hide(ctx)` would be well-typed at
the call site, and the closure that came back would hold a capability behind a
type that mentions none. Stopping exactly that is the point of the capture rule,
so the rule stays conservative here.

A type parameter escapes the restriction when it carries an **ordinary trait
bound**. A type is either part of the world or part of your data, never both, so
a `T: Eq` can never be instantiated at a context type. That is why
