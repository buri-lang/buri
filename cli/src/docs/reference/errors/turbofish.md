...
program either. No source accepts both readings and disagrees about them, and
the `::` was the price of an ambiguity that turned out not to be one.

Two spellings of one thing would be worse than either, so the old one is an
error rather than a second way to write it. The error carries the edit as bytes,
