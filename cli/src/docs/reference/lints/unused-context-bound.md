...
**A callee that asks for one of its own.** `io.println(ctx, …)` and
`str.format(ctx, …)` instantiate
...
written down in the checked tree.
...
`<C: Alloc + FsWrite + Io>` both claim the `+` between them — so removing several is
