---
title: Every declared type is used
severity: warning
message: "nothing uses `{name}`"
...
on the library's surface is public API, and this rule does not ask about it,
because the consumers this analysis can see are not the only ones there are. A
type declared under `testing/` is measured against `testing/lib.buri` the same
way.
