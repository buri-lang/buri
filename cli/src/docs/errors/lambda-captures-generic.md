...
opaque, and a generic body is checked once for every instantiation at once
...
type that mentions none. That is the whole point of the capture rule, so the
rule has to be conservative here.
...
so a `T: Eq` can never be instantiated at a context type — which is why
