...
//!
//! `main` dispatches through it, `cli::usage()` is printed from it, and
...
pub const FLAGS: &[Flag] = &[
    ...
    Flag {
        ...
        set: |f, v| {
            f.format = match v {
                Some("json") => Some(cli::Format::Json),
                Some("markdown") | Some("md") => Some(cli::Format::Markdown),
    ...
    Flag {
        ...
        set: |f, v| {
            f.error_format = match v {
                Some("json") => cli::ErrorFormat::Json,
                Some("human") | None => cli::ErrorFormat::Human,
                Some(other) => {
                    return Err(format!(
