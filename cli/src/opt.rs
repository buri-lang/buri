...
//! call graph here is exact (see `mono.rs`), so a decision taken from it is a
...
//! any more is left for `js::eliminate_dead` to drop by name.

use crate::hir::{self, Expr, ExprKind, PatKind, Stmt};
use crate::mono::{Func, Program};
...
impl Facts {
    pub fn collect(program: &Program) -> Facts {
        ...
        for (i, func) in program.funcs.iter().enumerate() {
            ...
            hir::walk(body, &mut |e| match &e.kind {
    ...
    /// (`codegen::expr`), so a body carrying one, pasted into a caller, would
...
fn inline_expr(
    ...
    // into that table (`hir::Body`), so nothing else can capture: no two
    ...
    // to be unnecessary are removed later, by the local cleanup in `js.rs`.
...
fn shift_pattern(p: &mut hir::Pattern, offset: u32) {
    match &mut p.kind {
...
/// Every sub-expression, mutably. The mirror of `hir::walk`, which cannot be
    ...
    fn a_body_containing_a_question_mark_is_never_inlined() {
        let risky = func(
            ...
            Some(e(ExprKind::Try {
                ...
                kind: hir::OptionOrResult::Result,
            })),
        );
        let main = func("main", vec![0], 1, Some(call(1, vec![local(0)])));
