...
use crate::diag::{Diagnostic, Diagnostics, FileId, SourceMap, Span};
use crate::stdlib;
use crate::types::ModuleId;
use crate::workspace::{is_test_only_path, ModuleKind, RuleKind, TargetId, Workspace};
...
pub struct ModuleData {
    ...
    pub pkg: Option<crate::workspace::PkgId>,
    pub disk: PathBuf,
}
...
impl<'a> Loader<'a> {
    ...
    fn load_std(&mut self, path: &str, span: Span) -> Option<ModuleId> {
        ...
        let parsed = crate::parse::parse_stdlib(self.map.text(file), file);
        self.diags.extend(parsed.errors);
        let role = if stdlib::is_platform_module(path) { Role::Platform } else { Role::Std };
    ...
    fn load_file(
        ...
        let parsed = crate::parse::parse(self.map.text(file), file);
        self.diags.extend(parsed.errors);
    ...
    fn load_imports(&mut self, id: ModuleId) {
        let imports: Vec<(String, Span)> = self.modules[id.index()]
            ...
            .filter_map(|item| match item {
                ast::Item::Import(i) => Some((i.path.clone(), i.path_span)),
                ast::Item::ReExport(r) => Some((r.path.clone(), r.path_span)),
                _ => None,
            })
            .collect();
    ...
    fn role_for(&self, path: &str) -> Role {
        if path.starts_with("core/") {
            return if stdlib::is_platform_module(path) { Role::Platform } else { Role::Std };
        }
    ...
    fn check_import_legality(
        &mut self,
        importer_path: &str,
        importer_pkg: Option<crate::workspace::PkgId>,
        role: Role,
        path: &str,
        span: Span,
    ) -> bool {
