...
pub fn cmd_lsp(_args: &cli::Args) -> i32 {
    ...
    match (method, id) {
        ...
        ("textDocument/formatting", Some(id)) => {
            let result = (|| {
                ...
                // `format::source` returns `None` for anything that does not
                // parse, so a file mid-edit is left alone rather than mangled.
                let formatted = crate::format::source(&text)?;
                if formatted == text {
                    return None;
                }
...
fn parse_diagnostics(path: &std::path::Path, text: &str) -> Value {
    let parsed = crate::parse::parse(text, crate::diag::FileId(0));
...
fn code_actions(state: &mut State, params: &Value) -> Value {
    ...
    for d in &lint.items {
        ...
        let Ok(Some(update)) = crate::gen::regenerate(&mut session, pkg) else { continue };
