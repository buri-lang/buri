...
pub fn emit(
    ...
    optimize::run(program, &optimize::Options::default());
    ...
    let out = generate::generate(program, tables, &opts);

    let missing = generate::check_intrinsics(&out.missing_intrinsics);
    if !missing.is_empty() {
    ...
    Ok(javascript::print(&stmts, !release))
}
