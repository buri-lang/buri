...
pub fn schema_order(message: &str) -> &'static [&'static str] {
    match message {
        ...
        "forbids" => &["tags"],
        "requires" => &["platforms"],
        "lint" => &["check_during_build", "fail_on_finding", "rules", "allow"],
        // The two messages whose fields are not written down: `rules` holds a
        // `default` and one bool per lint code, `allow` a list of declarations
        // per rule that can have one, and the codes are the catalogue's.
        // Written here as a list either would be a copy that could go stale,
        // and a lint whose field was forgotten is a lint a repository cannot
        // name.
        "rules" => crate::documentation::lints::rule_fields(),
        "allow" => crate::documentation::lints::allow_fields(),
        _ => &[],
    }
}
