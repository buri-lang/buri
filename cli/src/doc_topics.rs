...
//! assembled document — one line in `doc_assemble::DOCUMENTS`.
...
/// `every_grammar_keyword_is_a_keyword` holds it against `lex::Kw`.
...
mod tests {
    ...
    fn every_title_matches_its_first_heading() {
        for t in TOPICS {
            let headings = doc_md::headings(t.text);
