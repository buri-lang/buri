...
/// output.
///
/// The tail of `build::build_target` minus the workspace, the cache, and the
...
pub fn run_snippet_in(
    ...
    let analysis = analyze_snippet_in(ws, map, name, text, crate::compile::Role::Entry);
    ...
    let Some(entry) = analysis.checked.entry else {
        diags.push(
            crate::diag::Diagnostic::error(crate::diag::Span::NONE, "this example exports no `main`")
                .with_fix(
...
fn execute(name: &str, source: &str) -> Result<String, Diagnostics> {
    ...
    let fail = |msg: String, fix: &str| {
        ...
        d.push(crate::diag::Diagnostic::error(crate::diag::Span::NONE, msg).with_fix(fix.to_string()));
        d
    };
    ...
    let out = match Command::new(crate::testrun::js_runtime()).arg(&path).output() {
        Ok(o) => o,
        Err(e) => {
            return Err(fail(
                format!("cannot run {}: {e}", crate::testrun::js_runtime()),
