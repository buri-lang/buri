...
///
/// **A print is no exception**, and that is the whole of what "must-use is
/// total" costs. `Stdout` and `Stderr` answer `Result<(), IoError>` because a
/// closed pipe is a thing that happens, so nearly every line a program writes
/// is a `Result` and `io.println(ctx, x).ignore()` is a deliberate drop like
/// any other. It is reported like any other: the rule is that somebody
/// decided, and a program whose report is all prints is a program that decided
/// that many times.
