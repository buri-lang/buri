...
use crate::commands::add_skills::{self, Outcome};
...
pub fn generate(root: &Path) -> Result<Vec<(Outcome, PathBuf)>, String> {
    ...
    // the next must end up with the same `.claude/skills`.
    done.extend(add_skills::install(root)?);
    Ok(done)
}
...
mod tests {
    ...
    fn a_run_writes_the_whole_repository() {
        ...
        for skill in add_skills::SKILLS {
            let path = root.join(".claude/skills").join(skill.name).join("SKILL.md");
            assert_eq!(std::fs::read_to_string(&path).unwrap(), skill.text);
        }
        assert_eq!(
            done.len(),
            SCAFFOLD.len() + add_skills::SKILLS.len(),
