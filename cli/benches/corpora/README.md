...
They buy **byte-stability over time, and nothing else**. Two runs a year apart
compile the same bytes, so any difference between them is a difference in the
compiler rather than in the generator. The suite reports both readings of
`mixed` for that reason: when the compiler changes, the generated and the saved
rows move together; when the *generator* changes, only the generated one moves.

These are not the corpus. The headline scale is 100,000 lines — 3.5 MB of
source, in a git history whose whole size is 15 MB — so the harness generates
everything above 512 KiB and every profile not listed below on each run. Where
the bytes still have to be pinned at scale, the corpus gets a manifest in
`cli/benches/pinned/` and no source at all.
...
`main.buri` last by an explicit rule in `corpus.rs`. The generator emits `m0000`
… `m9999` and `--record` preserves that. A corpus whose modules do not sort into
dependency order will not load, and `--validate` is where you find out.
...
holds it to that. It *is* an exception to `BURI_BLESS`: the fix for a drift here
is `--record`, not laying the file out where it sits, so the test says so and
leaves the file alone (`GENERATED_NOT_BLESSED`). Blessing would write bytes no
generator wrote and leave `manifest.txt` pinning a corpus that is no longer on
disk.
...
writes, so a corpus that does not compile never lands. Overwriting needs
`BURI_BLESS=1`, because re-recording breaks a measurement series.
...
2. **Fix it by re-recording with a bumped `revision`**, in the same commit as
   the language change. `--json` carries `corpus_revision`, so a tracking script
   sees a break in the series rather than a step in it. The directory name stays
   put: a stable key with a visible discontinuity beats a new key nothing joins
   to.
3. **Delete a corpus you cannot re-record. Do not repair it.**
4. **Delete a corpus that has been stale for a release**, whatever the reason.
5. Bump `GENERATOR_REVISION` in `generate.rs` by hand whenever a change there
   ...
   legal and expected, so `--validate` prints it as a note and never as an
   error.
6. **A change to the formatter is a change to the generator.** `laid_out` is the
   ...
   somewhere else moves every corpus. The repository-wide format sweep notices
   first, and the fix is the same ceremony: bump `GENERATOR_REVISION`, re-record
   all eight, re-pin all forty, write the note in `design/PERFORMANCE.md` §6.
