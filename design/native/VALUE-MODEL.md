...
One test file, `cli/tests/backend_agreement.rs`, runs a corpus under both
...
| 8 | Float rendering | JS `Number#toString` | shortest round-trip (§11.4) | Must agree, character for character. The runtime implements Ryū rather than trusting a libc `printf`. The corpus is `native_float_parity.rs`'s 3.8 million doubles; the row here is the end-to-end variant. | `row_08_float_rendering` |
