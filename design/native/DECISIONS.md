...
It exists because the arguments are spread across seven documents and a reader
...
| The middle end owns tail-call elimination; no backend uses its own | CODEGEN-LLVM.md §5, CODEGEN-CRANELIFT.md §3.3 | `musttail`/`tailcc` and `return_call` are both viral, both incompatible with a C-ABI runtime, and both unnecessary once the tail-call graph is a DAG. |
...
| `backend-llvm` off by default; `backend-cranelift` and `backend-stencil` on | BUILD-AND-WATCH.md §2 | `cargo install buri` must not need a system LLVM. The other two need nothing that a machine able to link a native artifact does not already have. |
| The stencil backend is compiled in and never selected | ARCHITECTURE.md §4, CODEGEN-STENCIL.md | Selecting it is a decision about parity, and holding it to the same traits keeps that decision from becoming one about plumbing. |
