...
- **`Str` concatenation — `llvm/emit.rs`'s `concat`.** `str.concat` is
  open-coded rather than called. **The in-place path is the release backend's
  alone** since 2026-08-29: the backend that carried the other copy was removed,
  and the debug backend allocates unconditionally, which is observable in
  `core/alloc`'s counts and is recorded as a divergence rather than as a missing
  optimisation (CODEGEN-STENCIL.md §9, §13). The
