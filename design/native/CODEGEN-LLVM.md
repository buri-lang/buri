...
Each is a thing a frontend either did or did not do before LLVM was handed a
module.
...
- The call graph is exact: no dynamic dispatch anywhere in the language, so
  "unreachable" is a fact rather than an approximation.
...
`globaldce` and the inliner walk quickly over a module with no dead code, and the
...
`default<O2>` (§4) and is cheap when there is nothing to promote.

The counter-design — an `alloca` per local, `store` on definition, `load` on use,
and let mem2reg sort it out — is what §0's second instruction names. Its cost is
not the pass: every intermediate value passes through memory in the IR the
*inliner* and the early simplification passes see, so those passes take their
decisions on a worse program.
...
The instruction is about using memory as a stand-in for SSA values — an `alloca`
whose whole purpose is to be promoted back. This one is a genuine aggregate whose
fields are accessed by `getelementptr`, exactly as a C local struct would be, and
SROA splitting it into scalars afterwards is the optimization doing its job. The
entry block is what makes it eligible for that treatment at all.

### 2.4 There is no mutable-slot emulation, including for tail-call loops

The obvious place a frontend needs mutable slots is the loop
...
(`generate.rs`, `retarget_assigns`). In block-argument SSA it is not. The loop
header block takes the
...
block parameter carrying the member index. So the construct that would have
forced mutable-slot emulation is the one that most naturally produces phis. That
...
§0's third instruction. The effect system supplies most of this for free: **a
language where "does this function touch the world?" is a syntactic property of
its signature is a language that can answer LLVM's memory-effect questions
without an analysis.** SPEC 10.4's purity theorem is an attribute-emission rule.
...
are qualifiers the theorem already names:
...
- **"terminate without aborting"**: an abort writes to stderr and exits, which is
  ...
  `memory(argmem: read, inaccessiblemem: write)`. The **"terminate"** half is a
  *second* condition. It governs `willreturn` rather than `memory(...)`, `middle`
  does not compute it, and reading it as though it were the abort condition is
  the miscompile §3.6 records.
...
What the fourth and fifth rows add is the one thing the theorem does not talk
about: SPEC 10.4 is a statement about *observable effects*, and LLVM's
`memory(...)` is a statement about *bytes*. Reference counting writes bytes that
no Buri program can observe, and that difference is §3.2.1.
...
Without it, LLVM has to assume every call is a potential unwind edge, which
pessimizes everything downstream.
...
`readonly` on a parameter is not unconditional. The argument that it is — values
are immutable, in-place growth writes only past the end of a block whose count it
has just seen to be 1, and the count is not part of the value — is true about
Buri and is not what `readonly` means. LangRef:
...
promise about bytes. And LangRef's *Pointer Aliasing Rules* settle where the
count lives — a pointer formed by `getelementptr` **is** *based on* its operand,
and `argmem` is "accesses that are based on pointer arguments", so `incref`'s
store at `p - 16` for a parameter `p` is a write to argument memory. Three writes
in this language go through a parameter:
...
A function's `memory(...)` covers its callees' effects as well as its own, so the
condition is a whole-call-graph one. `backend/llvm/emit.rs`'s `observe` computes
it beside the allocation and abort bits, and `argument_based` decides *which*
memory a count sits in: a register projection of a parameter keeps the
parameter's provenance, a `load` — a `[T]`'s element — does not, and a block
parameter takes the conjunction of its incoming values, which matters for every
counted function because `middle::tail_calls` turns self-recursion into a loop.

The rule, which is what `backend/llvm/attrs.rs` implements:
...
`opt -passes=function-attrs` on the same shape — a `getelementptr i64 -16` from a
...
`readonly` on the parameter and `memory(readwrite, inaccessiblemem: none)`. LLVM's
own answer and this table are the same table.
...
- **The allocating row.** A function that allocates and then initializes the
  ...
  excuses the *allocator*, not its caller. `opt` infers
  `memory(write, argmem: none, inaccessiblemem: readwrite)` for exactly that
  body. `errnomem` comes with it: `malloc` sets `errno`.
...
The discipline survives for functions that touch no count, which on a
representative program is most of them: of twenty pointer parameters across twelve
functions, sixteen carried `readonly` before and ten after — the six that went are
the parameters of the three functions with a reference-counting plan — and no
function lost its `memory(...)` outright.

`cli/tests/native/llvm.rs` holds the two tests that keep this true. One scans
*every* emitted function for a store to `p - 16` where `p` is marked `readonly`,
or under a `memory(...)` whose `argmem` is not writable. The other runs one
program through `default<O0>` and `default<O2>` and requires the two to agree
about stdout, the exit code and `buri_rt_live_blocks`.
...
miscompile that shows up as a wrong answer months later.
...
would be *correct*. It is still not set, for a reason about the language rather
than about LLVM: VALUE-MODEL.md §11 records the SPEC 6.2 amendment describing two
backends whose overflow behaviour differs, and "two's-complement wrap" is a
description a program can be debugged against. `nsw` makes it "whatever the
optimizer inferred from the assumption that it never happened", which changes
between LLVM releases. A program that overflows is wrong either way; one of the
two answers can be reported in a bug and reproduced.
...
integer is already `i64`, which is the pointer width.
...
an index that `list.get` bounds-checked into an `Option` (`list.buri`). Unlike
`nsw`, the type system enforces the premise rather than assuming it away.
...
`MemoryEffects` builder gated on the LLVM version — and no call site writes a
literal. A hard-coded `0x55` is a silent miscompilation of the attribute on an
LLVM bump, which is the worst kind: the IR still verifies.

### 3.6 `willreturn` needs a proof, and the middle end does not have one

§3.1's table used to read *"a function `middle` proved terminates"*, and `middle`
proves nothing of the kind: `rc::infer_effects`'s columns are purity, abortability
and parkability, none of which is about coming back. The backend read the row as
`Purity::Pure` with no abort — and
...
recursive call **above the branch that guards it**. `default<O2>` took it and the
program recursed until the machine stack ran out; `default<O0>` printed the right
answer all along, which is what a false attribute looks like from outside.
...
returns"*. Divergence is observable here, so neither attribute may be emitted
without a proof.

**What the backend proves instead is "no cycle".** A function with no loop in its
control-flow graph, which can reach no cycle in the call graph, executes a bounded
number of instructions and returns. `emit::observe` carries the negation as
`Observed::may_diverge`, beside the allocation, abort and reference-count bits and
for the same reason: a declaration in one unit and a definition in another must
carry the same attributes. Its seed is `emit::reaches_a_cycle`, a peel of the call
graph, and it has to be a separate walk — `observe`'s propagation is a *least*
fixpoint from an optimistic start, so a function whose only unproven callee is
itself would converge on "proven".

The gap is named rather than hidden: a `for` over a list terminates, a recursion
down a `[T]` terminates, and neither is proven here. Proving them wants a
decreasing-measure analysis in `middle`, after which the bit belongs in
`ir::Facts` beside `can_abort`. Until then a loop costs `willreturn`, and the
trade is not close — the attribute buys hoisting and dead-call elimination, and a
wrong one buys an unbounded recursion.
...
aggressiveness and in inline thresholds, and costs code size and compile time for
a gain that is workload-dependent and usually small.

**A pipeline string, not a hand-assembled pass list.** A custom pipeline is a list
somebody has to re-derive at every LLVM bump, against a pass manager whose pass
names and orderings are internal. `default<O2>` is the pipeline LLVM's own
developers regression-test, and a redundant pass on a module it has nothing to do
...
The legacy `PassManager` is deprecated in inkwell 0.9.0 and gated
`#[llvm_versions(..=16)]` — it does not exist on LLVM 17 and above, so there is no
choice to make. `Module::run_passes` requires a `&TargetMachine`, so the target is
initialized before any optimization even in a hypothetical IR-only run.
...
**`musttail` is not needed at all**, and neither is `tailcc`.
...
function (`tail_calls.rs`). Both are exact, and both reach LLVM as ordinary CFG
loops with phis (§2.4).
...
honour it calls `report_fatal_error`, so an LLVM version or a target that tightens
a rule turns a working program into a compiler crash. On AArch64 — half the
platforms here — it does *not* bypass `isEligibleForTailCallOptimization` and
hard-errors on `byval` parameters, on SME/streaming mode changes, and when
outgoing stack arguments exceed the caller's incoming area, and there is an open
miscompile in exactly this combination (`llvm#167181`).

**What `tailcc` would cost.** It relaxes most of the AArch64 conditions and it is
**viral**: the convention must match numerically at every call site, so adopting
it for one function adopts it for its whole call graph. `cli/runtime` is a C-ABI
archive (VALUE-MODEL.md §10), so the 203-function intrinsic surface would need
C-ABI shims. It also forbids varargs and is not the platform C ABI, so a Buri
artifact could no longer be linked against anything.
...
nothing — this compiler generates both sides of every Buri call, and
VALUE-MODEL.md §5.1 already flattens aggregates so no convention has to classify
one.
...
- **Cold blocks are marked cold.** Every abort path, every `decref` free path
  ...
  where it does not return. It is the highest-value item here, because reference
...
- **The unit is a source module** (ARCHITECTURE.md §5.1), so functions that call
  ...
  emission order is the monomorphization worklist's — deterministic, and derived
  from the reachability walk out of the entry point rather than from a hash order
  (`monomorphize.rs`). That is a free first approximation of a call-order layout,
  since a callee is instantiated when its caller is reached. A measured layout,
  from `--explain` counts or a profile, would replace the ordering and not the
  partition.
...
DWARF via inkwell's `DebugInfoBuilder`. It is cheaper here than in the debug
backend (CODEGEN-STENCIL.md §11) because LLVM emits the sections; the frontend
supplies `DILocation`s from `typed::Expr::span`, which every node already carries,
and `DISubprogram`s from `Func::debug_name` (`monomorphize.rs`).
...
The prefix already reaches an object without waiting for debug info: it is the
head of a unit's module name (`emit_selected`), and LLVM emits a module's
source-file name as a `.file` directive on every ELF target, so it lands in
`.symtab` as an `STT_FILE` symbol today. That is why it is a term of the
`codegen` key (ARCHITECTURE.md §6.2).
...
- **Exactly one supported LLVM at any moment.** Not a range, not a minimum, not a
  set of `#[cfg]`-selected encodings. `strict-versioning` is what would enforce
  that in the build rather than by a promise, and §8.3 records why it is not on.
- **The pin is the latest LLVM that inkwell *and* the flake's nixpkgs both
  ...
  do — inkwell has `llvm22-1` and `nixos-25.05` stops at 21.
...
- **Bumping it is a chore, not an event.** No deprecation window, no transition
  period, no compatibility shim; the checklist is §8.2.
- **Multi-version support is refused, permanently.** The `memory(...)` bitmask in
  §3.5 is why: two LLVMs means the *same* attribute encoded two ways and a test
  matrix that has to build both, for a benefit — a contributor keeping an older
  LLVM — that `nix develop` already delivers.

Neither simpler policy works. "The flake leads" would forbid a bump the backend
needs; "the backend leads" prices a nixpkgs bump — which moves `cargo`, `bun` and
`elan`, and therefore every artifact this toolchain produces — into a routine
chore. Tracking latest-available means the pin moves when a nixpkgs bump that was
happening anyway brings a newer LLVM along.
...
inkwell supports LLVM 12 through 22 with zero lag behind stable LLVM, so 22 would
be the newest valid pin on its side alone. 21 is what both sides carry:
...
- `nixpkgs` on the pinned `nixos-25.05` provides `llvmPackages_9` and `_12`
  ...
  **There is no `llvmPackages_22` on 25.05** — checked against the locked
  revision, not the channel name. Pinning 22 would mean bumping the flake's
...
dependency bar: inkwell's `llvm21-1` expands to `llvm-sys-211` and nothing else,
so reaching the flag would mean a second direct dependency on `llvm-sys` for one
setting. Standing in for it: `Backend::identity()` puts the LLVM version into
every release `codegen` key, so two toolchains built against different LLVMs never
share a cached object, and `llvm-config --version` is what a contributor checks.
`cli/Cargo.toml` and BUILD-AND-WATCH.md §3.4 both say so.
...
which is fine — the linker's version does not have to match the compiler's.
