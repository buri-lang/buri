...
`Value` is shaped to line up with `exhaust.rs`'s `Ctor` (`exhaust.rs:21`) one
...
`Value.single`, exactly as they share `Ctor::Single` -- each has exactly one
constructor, and the algorithm never needs to tell them apart.

`Value.opq` stands for values of function, context, and type-parameter type. No
pattern can inspect one, so its internal structure is irrelevant here; what
matters is that such values exist and that `allCtors` reports *no* enumerable
...
inductive Value where
  ...
  | variant : TyConId → Nat → List Value → Value
  ...
  `Ctor::Lit` exists and why completeness is conditional (SPEC 7.3). -/
  | lit : String → Value
...
theorem Value.ind' {motive : Value → Prop}
  ...
  fun v =>
    Value.rec (motive_1 := motive) (motive_2 := fun vs => ∀ v ∈ vs, motive v)
      variant single bool array lit opq
...
inductive HasType (S : Sig) : Value → Ty → Prop where
  | variant {c i args vs} :
      i < S.variantCount c →
      Forall₂ (HasType S) vs (S.variantFieldTys c i args) →
      HasType S (.variant c i vs) (.con c args)
  | «struct» {c args vs} :
      S.isStruct c = true →
      Forall₂ (HasType S) vs (S.structFieldTys c args) →
  ...
  | bool {c b} :
      S.primOf c = some .bool →
      HasType S (.bool b) (.con c [])
  | lit {c p s} :
      S.primOf c = some p → p ≠ .bool →
      HasType S (.lit s) (.con c [])
  | array {vs e} :
      Forall₂ (HasType S) vs (List.replicate vs.length e) →
      HasType S (.array vs) (.array e)
  | fnv {ps r} : HasType S .opq (.fn ps r)
  | ctxv {k} : HasType S .opq (.ctx k)
  | paramv {n} : HasType S .opq (.param n)
...
theorem HasType.con_enum_inv {S : Sig} {c args v} (hen : S.isEnum c = true) :
    (S ⊢ᵥ v : .con c args) →
    ∃ i vs, v = .variant c i vs ∧ i < S.variantCount c ∧
            Forall₂ (HasType S) vs (S.variantFieldTys c i args) := by
  intro h
  cases h with
  | variant hlt hf => exact ⟨_, _, rfl, hlt, hf⟩
  | «struct» hst _ => simp [Sig.isEnum_isStruct hen] at hst
  | bool hp => simp [Sig.isEnum_primOf hen] at hp
  | lit hp _ => simp [Sig.isEnum_primOf hen] at hp

theorem HasType.con_struct_inv {S : Sig} {c args v} (hst : S.isStruct c = true) :
    (S ⊢ᵥ v : .con c args) →
    ∃ vs, v = .single vs ∧ Forall₂ (HasType S) vs (S.structFieldTys c args) := by
  intro h
  cases h with
  | «struct» _ hf => exact ⟨_, rfl, hf⟩
  | variant hlt _ => simp [Sig.isStruct_variantCount hst] at hlt
  | bool hp => simp [Sig.isStruct_primOf hst] at hp
  | lit hp _ => simp [Sig.isStruct_primOf hst] at hp

theorem HasType.con_bool_inv {S : Sig} {c args v} (hp : S.primOf c = some .bool) :
    (S ⊢ᵥ v : .con c args) → ∃ b, v = .bool b := by
  intro h
  cases h with
  | bool _ => exact ⟨_, rfl⟩
  | lit hp' hne => rw [hp] at hp'; exact absurd (Option.some.inj hp').symm hne
  | variant hlt _ => simp [Sig.primOf_variantCount hp] at hlt
  | «struct» hst _ => rw [Sig.isStruct_primOf hst] at hp; simp at hp

/-- A primitive that is not `Bool` -- an integer, float, string, or char. These
are the types `allCtors` reports `none` for: their constructor sets are too
...
theorem HasType.con_lit_inv {S : Sig} {c args v p}
    (hp : S.primOf c = some p) (hne : p ≠ .bool) :
    (S ⊢ᵥ v : .con c args) → ∃ s, v = .lit s := by
  intro h
  cases h with
  | lit _ _ => exact ⟨_, rfl⟩
  | bool hp' => rw [hp] at hp'; exact absurd (Option.some.inj hp') hne
  | variant hlt _ => simp [Sig.primOf_variantCount hp] at hlt
  | «struct» hst _ => rw [Sig.isStruct_primOf hst] at hp; simp at hp

theorem HasType.tuple_inv {S : Sig} {ts v} :
    (S ⊢ᵥ v : .tuple ts) → ∃ vs, v = .single vs ∧ Forall₂ (HasType S) vs ts := by
  intro h; cases h with | tuple hf => exact ⟨_, rfl, hf⟩

theorem HasType.unit_inv {S : Sig} {v} : (S ⊢ᵥ v : .unit) → v = .single [] := by
  intro h; cases h with | unit => rfl

theorem HasType.array_inv {S : Sig} {e v} :
