...
safety, the purity/capability theorem, exhaustiveness correctness, and inference
soundness.

**Exhaustiveness is done, and so is type safety for the core.** The two headline
results are
...
place: `cli/tests/vectors/lean.rs` reads checked-in vectors and needs no Lean.
...
* **`Pattern.WellFormed` is too weak.** It never says the head constructor
  *belongs to* the type, and it cannot, because for a nullary constructor the
  ...
  judgment, which `semantics/patterns.rs` computes and the progress theorem
  never needed. (`isUseful_false_covers` holds for ill-typed patterns, because
  a pattern that matches nothing only ever *loses* coverage.)
* **Inhabitation.** Even with pattern typing, the incomplete-constructor-set
  ...
  stops being stylistic. At a non-enumerable type it also needs a literal
...
one), no guarded arms (a guarded arm covers nothing for exhaustiveness
purposes, so a `match` whose only matching arm is guarded *can* get stuck), and
no builtins (`semantics/builtins.rs` is axioms in any model of this kind).
...
* **`match` and `let` carry semantic premises.** The typing rules require
  ...
  `exhaustive_correct_unbounded` a job: bridging the syntactic condition the
  checker decides to the semantic one the rule states.
...
* **`backend/js/generate.rs` / `backend/js/javascript.rs`.** The largest
  unproved surface, with a known gap: `Prim::is_bigint()` is `false` for every
  type and `EXACT_INTEGER_LIMIT` is `2^53 - 1`, so `I64`/`I128` arithmetic is
  inexact above `2^53`. A proof about `I128` arithmetic describes a language
