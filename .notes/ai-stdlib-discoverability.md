...
*uncommitted* working tree of `monorepo-buri`, written 2026-09-03 by session
...
**The one-line answer.** The AI mostly *did* consult the docs — 211 `buri docs`
...
`a480ac37` L6) says: *"use `buri docs <topic>` liberally … `buri docs search
<words>`. Docs are generated from the real stdlib source and are
authoritative."* It then lists nine modules — and those nine are what got read.
...
1. **Hex, twice (never asked).** `a224933f` L137/L141 writes `prng.buri` and
   ...
   never once mentions `toHex`. It *was* in doc-reading mode at that moment — L135
   is `buri docs language/lexical | grep hexadecimal`, looking up hex *literals*.
...
4. **`.ignore()` deliberately routed around.** The strongest incident.
   ...
   discarded-result`, which explains that `.ignore()` is *precisely what the lint
   reports*. The agent, held to a "lint clean" gate, then wrote
...
5. **A compiler bug forced hand-written comparators, and they stayed
   ...
   `compareBools`, `compareFloats`. Note `compareStrs` *does* delegate
   ...
   **workaround written without a second lookup**, compounded by *one operation,
   two addresses* (`a.compare(b)` vs `order.int(a, b)`).
...
6. **The wrong page, one page away.** `ac80e423` L237 tries `bytes.show(ctx)` in
   ...
   [no-such-method] ``` — whose fix line says *"buri docs <module> lists the
   methods it has"*. At L240 it opens `buri docs core/list.join`, and at L242
...
8. **A diagnostic that hallucinated an absence for us** (August/codex, but the
   ...
   namespace reports the *namespace* missing: `fs.appendBytes(...)` →
   ...
   diagnostic"), with the workaround *"check `buri docs core/fs` manually to
   distinguish a missing member from a missing namespace"* — and, before working
...
**"Did not look" is not the diagnosis — "did not look *for this*" is.** The docs
...
1. **Ship the reading list, because the reading list is what worked.** The only
   ...
   a hunch. Put a *stdlib map* — every `core/*` module, one line each, plus the
   ...
   first line of it. *Evidence:* 2/2 agents told to read `core/order` used it;
...
2. **Lints that name the library — the only check that fires where the mistake
   ...
   `1`/`0` is a wire byte (`storage/codec.buri:283` is legitimate). *Evidence:*
...
3. **Fix the `discarded-result` incentive; it is an own-goal.** The rule makes
   ...
   writes the four-line `match` instead, which is *not* reported — so the rule
   ...
   `match` form is worse. *Evidence:* `a224933f` L125–126, then six matches.
...
4. **Generated stdlib docs — do it, but not for this.** Honest evaluation: at
   ...
   it will not on this evidence change rewrite behaviour. The *one* search change
...
5. **`buri init` should write an AGENTS.md.** It writes `.agent/skills/*` and no
   ...
   every agent by hand. The file below is the content. *Evidence:*
...
6. **"Did you mean `core/list.maxBy`" from the LSP — not feasible for this
   ...
   hover cannot reach a heredoc. The same idea *is* feasible where agents do
...
7. **Make a missing member say so.** `fs.appendBytes` reporting *"there is
   nothing named `fs` in scope"* tells an agent the module is absent when a member
...
## 3. What to tell the AI

Draft for the AGENTS.md / CLAUDE.md that `buri init` should write:

```markdown
## Buri

- Read `buri docs` (no arguments) once before writing any Buri. It is the whole
  index — every module the toolchain ships — and it costs one call.
- Assume the library already has it. Before writing a comparator, a hex digit
  table, a padding or alignment helper, a discard, a random number, a formatter
  or a path join, run `buri docs search <words>` and read
  `buri docs core/<module>` in full.
- These exist and are the ones people rewrite by accident: `order.int`,
  `order.bool`, `Order.then`, `Bool.toInt`, `str.padStart`/`padEnd`,
  `bytes.toHex`/`fromHex`, `char.fromDigit`, `num.toHex`, `str.toRadix`,
  `Result.ignore`, `list.sortBy`, `Saturating`, `.show(ctx)`, `context Fixture`.
- Drop a `Result` you do not care about with `.ignore()`. `buri lint` reports it
  on purpose — that report is the rule working, not a finding to route around.
  Never write `match (r) { .Ok(_) => (), .Err(_) => () }`.
- A lint that names a standard library function (`hex-digit-table`,
  `time-unit-conversion`) means you rewrote something that exists. Take the fix.
- Do not copy a helper out of a sibling package. If two packages need it, it is
  a library: either `core/*` has it, or make a package for it.
- If the library truly lacks something, leave `// TODO(buri): <what is missing>`
  at the call site and say so in your report — do not quietly ship your own.
- A method lives in its receiver's module and needs no import; the same
  operation may also exist as a free function elsewhere (`a.compare(b)` and
  `order.int(a, b)`). Not finding a method is not evidence of absence.
```
