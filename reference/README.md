...
Every paper consulted while building the Buri toolchain, with a link to a copy
its author, arXiv, or the institutional repository publishes — nothing here is
behind a paywall. This is a reading list, not a library: the papers are other
people's work and this repository links them rather than re-hosting them, so a
link is what each row carries.

Cite these from the code and the design docs by author and title, and point at
this file — the way `cli/src/formatting.rs` and `design/native/MEMORY.md`
already do. A citation that survives is one a reader can search for; a copy is
not needed for that, and a path into this directory would be.
...
| *Reference Counting with Reuse in Roc* (MSc thesis) | Jasper Teeuwissen (Utrecht University) | 2023 | [studenttheses.uu.nl](https://studenttheses.uu.nl/handle/20.500.12932/44634) | The same algorithm in a shipped compiler, with the measurements of where reuse pays. Named in `design/native/MEMORY.md` §5.3 as the sanctioned direction (Roc-style Perceus with in-place mutation at refcount 1). |
| *Simple and Efficient Construction of Static Single Assignment Form* | Matthias Braun, Sebastian Buchwald, Sebastian Hack, Roland Leißa, Christoph Mallon, Andreas Zwinkau | 2013 | [pp.ipd.kit.edu](https://pp.ipd.kit.edu/uploads/publikationen/braun13cc.pdf) | The algorithm `cranelift-frontend`'s `FunctionBuilder` runs, and the reason the Cranelift backend never used it while it was here: `middle::ir` is already block-argument SSA, so there is nothing to construct (`design/native/ARCHITECTURE.md` §2.1, `CODEGEN-STENCIL.md` §0). |
...
| *Ryū: Fast Float-to-String Conversion* | Ulf Adams | 2018 | [ACM PLDI'18 open access](https://dl.acm.org/doi/10.1145/3192366.3192369) | The shortest-round-trip digit generation `design/native/VALUE-MODEL.md` §11.4/§12 row 8 requires of the native runtime. `cli/runtime/fmt.rs` documents why v1 leans on `core::fmt` (Grisu3 with a Dragon4 fallback) instead of hand-rolling this, and where a Ryū fast path would drop in. |
| *Computationally Easy, Spectrally Good Multipliers for Congruential Pseudorandom Number Generators* | Guy L. Steele Jr., Sebastiano Vigna | 2020 | [arXiv:2001.05304](https://arxiv.org/abs/2001.05304) | The 64-bit multiplier `K` in `cli/src/hash.rs`, taken with rustc's `FxHasher` and cited there by name. |
| *Copy-and-Patch Compilation* | Haoran Xu, Fredrik Kjolstad | 2021 | [arXiv:2011.13127](https://arxiv.org/abs/2011.13127) | The technique the debug backend is built on (`design/native/CODEGEN-STENCIL.md`), and the source of the "Cranelift is slow" numbers that circulate; reading it is what established those were measured against Wasmtime 0.26 and did not describe the version this repository pinned. Backs the debug/release split in `design/native/ARCHITECTURE.md` §4. |
...
- Cranelift: `cranelift/docs/ir.md`, `cranelift/frontend/src/ssa.rs`,
  ...
  Read for the backend this toolchain carried until 2026-08-29; kept because
  `middle::ir`'s block-argument form came from CLIF and the reading is why.
- Chris Fallin, *ægraphs: acyclic e-graphs* (blog post and EGRAPHS 2023 slides) —
  what `opt_level = "none"` skipped, and half of why the dev profile never left
  it: <https://cfallin.org/blog/2026/04/09/aegraph/>,
...
implementation that already does them correctly, so they are not in the table
above:
