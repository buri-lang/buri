...
| *Perceus: Garbage Free Reference Counting with Reuse* | Alex Reinking, Ningning Xie, Leonardo de Moura, Daan Leijen | 2021 | [Microsoft Research](https://www.microsoft.com/en-us/research/publication/perceus-garbage-free-reference-counting-with-reuse/) | The own/borrow elision and the reuse analysis in `cli/src/compiler/middle/rc.rs`. `design/native/MEMORY.md` §5.2–5.3 cites it directly, and its FBIP framing is where §5's growth path comes from. |
| *Reference Counting with Reuse in Roc* (MSc thesis) | Jasper Teeuwissen (Utrecht University) | 2023 | [studenttheses.uu.nl](https://studenttheses.uu.nl/handle/20.500.12932/44634) | The same algorithm in a shipped compiler, measuring where reuse pays. `design/native/MEMORY.md` §5.3 names it as the sanctioned direction: Roc-style Perceus with in-place mutation at refcount 1. |
| *Simple and Efficient Construction of Static Single Assignment Form* | Matthias Braun, Sebastian Buchwald, Sebastian Hack, Roland Leißa, Christoph Mallon, Andreas Zwinkau | 2013 | [pp.ipd.kit.edu](https://pp.ipd.kit.edu/uploads/publikationen/braun13cc.pdf) | The algorithm `cranelift-frontend`'s `FunctionBuilder` runs. The Cranelift backend never used it while it was here, because `middle::ir` is already block-argument SSA and there is nothing left to construct (`design/native/ARCHITECTURE.md` §2.1, `CODEGEN-STENCIL.md` §0). |
| *Tilting at Windmills with Coq: Formal Verification of a Compilation Algorithm for Parallel Moves* | Laurence Rideau, Bernard Serpette, Xavier Leroy | 2008 | [xavierleroy.org](https://xavierleroy.org/publi/parallel-move.pdf) | The theory behind parameter rebinding in `cli/src/compiler/middle/tail_calls.rs`. Rewriting a self tail call to a loop *is* a parallel move, and the read-before-overwrite and cycle-breaking rules are this paper's. CompCert ships it as `Parmov`. |
...
| *Tail Recursion Modulo Context — An Equational Approach* | Daan Leijen, Anton Lorenzen | 2022 | [Microsoft Research (MSR-TR-2022-27)](https://www.microsoft.com/en-us/research/publication/tail-recursion-modulo-context-an-equational-approach/) | Research input, not implemented. Koka's generalisation of the row above, and the reason the destination-passing shape it needs fits the Perceus reuse already in `middle/rc.rs`. |
...
- Cranelift: `cranelift/docs/ir.md`, `cranelift/frontend/src/ssa.rs`,
  ...
  `middle::ir`'s block-argument form came from CLIF, and this reading is why.
...
The code cites these by algorithm name, at second hand through an implementation
that already gets them right, so they stay out of the table above:
